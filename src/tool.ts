...
export class EditImageTool implements vscode.LanguageModelTool<IEditImageInput> {
  ...
  async prepareInvocation(
    ...
    return {
      invocationMessage: `Editing image with ${providerName}: "${prompt}"`,
      confirmationMessages: {
        title: 'Edit Image',
        message: new vscode.MarkdownString(
          `**Edit prompt:** ${prompt}` +
  ...
  async invoke(
    ...
    try {
      const result = await vscode.window.withProgress(
        ...
        async (progress, progressCancelToken) => {
          progressCancelToken.onCancellationRequested(() => abortController.abort());
          progress.report({ message: 'Loading source image…' });
          const res = await editAndSaveImage(this.context, {
            prompt,
      ...
      return new vscode.LanguageModelToolResult([
        new vscode.LanguageModelTextPart(
          `Image edited successfully and saved to \`${result.relativePath}\`.\n\n` +
            `You can insert it into your document with this Markdown snippet:\n\n` +
            `${result.markdownLink}\n\n` +
            `Call metrics:\n` +
            `- Provider: ${result.metrics.providerId}\n` +
      ...
      logDetailedError('Copilot tool image edit failed', err);
      if (abortController.signal.aborted) {
        return new vscode.LanguageModelToolResult([
          new vscode.LanguageModelTextPart('Image editing cancelled.'),
        ]);
      }
      const message = toUserErrorMessage(err);
      return new vscode.LanguageModelToolResult([
        new vscode.LanguageModelTextPart(
          `❌ Image editing failed: ${message}\n\n` +
            `Run **ImageGen: Set API Key** if needed. See the **ImageGen** output channel for details.`,
        ),
      ]);
    }
